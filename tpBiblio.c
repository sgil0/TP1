...
switch(chx)
	...
	case 3 : printf("Saisir le titre du livre : ");
			scanf("%s",&ptrT);
			reponse = rechercheLivreTitre(&B,ptrT);
				if (reponse>0) printf("\n Ce livre est présent %d fois dans la bibliothèque", reponse);
					else printf("\n Ce livre n'est pas dans la bibliothèque");
			break;

	case 4 : printf("Saisir l'auteur des livres: ");
			scanf("%s", &ptrA);
			afficheLivreAuteur(&B, ptrA);	
			break;
			
	case 5 : printf("Saisir le titre du livre : ");
			scanf("%s",&ptrT);
			reponse = supprimerLivre(&B,ptrT);
			if (reponse>0) printf("\n Ce livre e été supprimé de la bibliothèque");
					else printf("\n Ce livre n'est pas dans la bibliothèque");
			break;
	}
